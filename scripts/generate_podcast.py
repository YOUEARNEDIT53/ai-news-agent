...
DIGEST_EMAIL_TO_RAW = os.getenv('DIGEST_EMAIL_TO', 'youearnedit@gmail.com')
# Support multiple recipients (comma-separated)
DIGEST_EMAIL_TO = [email.strip() for email in DIGEST_EMAIL_TO_RAW.split(',') if email.strip()]
DIGEST_EMAIL_FROM = os.getenv('DIGEST_EMAIL_FROM', 'AI News Agent <onboarding@resend.dev>')
...
def send_podcast_email(audio_path, date):
    ...
    if response.status_code == 200:
        print(f"Podcast email sent successfully to {', '.join(DIGEST_EMAIL_TO)}")
        return True
    else:
